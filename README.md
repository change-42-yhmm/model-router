...
- 支持 `auto`（默认）、`speed` 与 `quality` 偏好；偏好只在安全、任务能力和验收条件允许的范围内生效。
...
路由请求可传入 `preference: "auto" | "speed" | "quality"`，默认是 `auto`。约束优先级固定如下：**安全/授权 > 任务能力与验收 > 用户偏好 > 成本与时延**。

- `speed` 只会降低低风险、范围明确任务的候选档位；高风险、深度推理或验收要求不会被降级。
- `quality` 会把低风险 `fast` 任务优先提升到 `balanced` 的首轮候选，但不会无依据地全部使用 `deep`。

仪表盘的「选择依据」页提供三种模式的交互式对比，用于理解其收益与受约束的边界。
