...
**当前版本：v0.2.0 — 运行前模型证据检查。** 自动路由会在执行前刷新官方来源；价格/能力快照过期、来源不可用或来源内容发生未复核变化时，会降级为人工批准，而不是按旧目录自动执行。
...
- 为 CLI 和本地网关新增运行前官方证据刷新。
- 检查模型价格/能力快照、官方来源新鲜度和内容哈希变化。
- 发现未复核变化时，自动执行降级为 `awaiting_approval`；更新目录并递增 `modelCatalogRevision` 后恢复自动路由。
- 更新 GPT‑5.6 Luna / Terra 的 2026-07-30 官方降价快照。
